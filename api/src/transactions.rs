...
use aptos_block_executor::task::TransactionOutput as OtherTransactionOutput;
use aptos_types::PeerId;
use cfx_evm::vm::Exec;
use hyper::header::{OccupiedEntry, VacantEntry};
...
use aptos_block_executor::block_executor::vm_wrapper::AptosExecutorTask;
use aptos_block_executor::executor::{WorkerType, ExecutionContext, ExecutionSpace, ExecutionStatusMultiWorker, ManagerSender, TwoWayChannelEndpoints, TwoWayChannels, WorkerSender};
use aptos_block_executor::task::ExecutorTask;
use aptos_block_executor::view::LatestViewArc;
use aptos_block_executor::{task::ExecutionStatus as ExecStatus, executor::PreprocessedTransaction, view::LatestView};
use aptos_executor::block_executor::BlockExecutor;
use aptos_block_executor::cached_state_view::CachedStateView;
use aptos_types::write_set::WriteSet;
use move_core_types::vm_status::VMStatus;
use aptos_block_executor::block_executor::AptosTransactionOutput;
use cfx_primitives::transaction::TxType;
use jsonrpsee::core::Error;
use cfx_primitives::TransactionWithSignature as EthTransaction;
use crate::{
...
use aptos_types::transaction::{self, TransactionOutput};
...
pub(crate) fn gen_li_with_sigs(
    block_id: aptos_crypto::hash::HashValue,
    root_hash: aptos_crypto::hash::HashValue,
    version: Version,
    ...
    let ledger_info = aptos_types::ledger_info::LedgerInfo::new(
        block_info,
        aptos_crypto::hash::HashValue::zero(), /* consensus_data_hash, doesn't matter */
    );
...
impl TransactionsApi {
    ...
    #[oai(
        path = "/stress_test_move_single_thread_ignore_types",
        method = "post",
        operation_id = "stress_test_move_single_thread_ignore_types",
        tag = "ApiTags::Transactions"
        )]
        async fn stress_test_move_single_thread_ignore_types(
            &self,
        ) -> SubmitTransactionResult<PendingTransaction> {
            let _result = self.context.benchmark_mutex.lock().await;
    
            info!("Running experiment using Aptos REST API");
            let bytes = std::fs::read("./api/src/move_transactions.txt").unwrap();
            let mut txns: Vec<transaction::Transaction> = Vec::with_capacity(100000);
            let mut bytes_read: usize = 0;


            // let mut previous_nonce_per_space: [Option<u64>; 2] = [None, None];  
            // let mut no_setup_tx_ethereum = false;
            // let mut no_setup_tx_aptos = false;

    
            while bytes_read < bytes.len() {
                let txn: SignedTransaction = self.read_tx_partially_ignoring_tx_type_padding(
                    &bytes, 
                    &mut bytes_read,
                );
                // println!("{}", txn.sequence_number());
                txns.push(transaction::Transaction::UserTransaction(txn));
            }

    
            let executor: BlockExecutor<AptosVM> = BlockExecutor::new(
                Arc::try_unwrap(
    ...
    
            {
                ...
                for block in txns.chunks(2500) {                
                    let mut block_fixed = block.to_vec();
                    block_fixed.push(transaction::Transaction::StateCheckpoint(aptos_crypto::HashValue::random()));
                    let block_length = block_fixed.len();
                    let block_id = aptos_crypto::HashValue::random();
                    let output: StateComputeResult = executor.execute_block(
                        (block_id, block_fixed), 
                        executor.committed_block_id(), 
                        // &channels,
                        // &mut previous_nonce_per_space
                    ...
                    let current_version = self
                        ...
                        .ledger_info()
                        .version();
                    info!("block size {}, current version {}, block hash {}", block_length, current_version, block_id);
                    let ledger_info_with_sigs = gen_li_with_sigs(block_id, output.root_hash(), current_version + block_length as u64);
                    // TODO: fix race condition in case the block size is small enough for it to be executed before the previous block gets processed / voted on.
                    // Potential fixes: add sleep statement / additional mutex in the beginning of the experiment. 
                    // Look at buffer_manager.rs -> fn start() and .aptos/testnet/validator.log for more details
                    executor
            ...
            // mock response; we care about the time
            SubmitTransactionResponse::try_from_bcs((
                                                    (),
                                                    &self.context.get_latest_ledger_info::<SubmitTransactionError>().unwrap(),
    ...
    #[oai(
        path = "/stress_test_move_single_thread",
        method = "post",
        operation_id = "stress_test_move_single_thread",
        tag = "ApiTags::Transactions"
        )]
        async fn stress_test_move_single_thread(
            &self,
        ) -> SubmitTransactionResult<PendingTransaction> {
            let _result = self.context.benchmark_mutex.lock().await;
            ...
            while bytes_read < bytes.len() {
                let txn: SignedTransaction = self.read_tx_without_tx_type_padding(
                    &bytes,
                    &mut bytes_read,
                );
                // println!("{}", txn.sequence_number());
    ...
    #[oai(
    path = "/stress_test_move",
    method = "post",
    operation_id = "stress_test",
    tag = "ApiTags::Transactions"
    )]
    async fn stress_test_move(
        &self,
    ) -> SubmitTransactionResult<PendingTransaction> {
        let _result = self.context.benchmark_mutex.lock().await;
        let (worker_sender, manager_reciever) = mpsc::channel::<Vec<ExecutionStatusMultiWorker>>();
        let move_manager_sender = self.gen_move_handlers_and_spawn_worker(worker_sender.clone());
        let eth_manager_sender = self.gen_eth_handlers_and_spawn_worker(worker_sender.clone());
        let cross_manager_sender = self.gen_cross_handlers_and_spawn_worker(worker_sender);
        let channels = TwoWayChannels {
            move_channel: (move_manager_sender, &manager_reciever),
            eth_channel: (eth_manager_sender, &manager_reciever),
            cross_channel: (cross_manager_sender, &manager_reciever)
        };

        info!("Running experiment using Aptos REST API");
        let bytes = std::fs::read("./api/src/move_transactions.txt").unwrap();
        let mut txns: Vec<transaction::Transaction> = Vec::with_capacity(510000);
        let mut bytes_read: usize = 0;

        // 0xuki: should we create a separate data structure of NPC accounts to track nonces or should we put everything inside one DS?
        // Ideally, we should put everything in one data structure to avoid searching for edge cases when changing the code of the nonce tracking in the executor
        let mut previous_nonce_per_address: HashMap<AccountAddress, u64> = HashMap::new(); // nonces of the deploying account
        let mut no_setup_tx_ethereum = false;
        let mut no_setup_tx_aptos = false;

        println!("Reading transactions from file...");
        while bytes_read < bytes.len() {
            let txn: SignedTransaction = self.read_tx_with_tx_type_padding(
                &bytes,
                &mut bytes_read,
                &mut previous_nonce_per_address,
                &mut no_setup_tx_ethereum,
                &mut no_setup_tx_aptos
            );
            // println!("{}", txn.sequence_number());
            txns.push(transaction::Transaction::UserTransaction(txn));
        }
        println!("Finished reading txs from file!");

        println!("Previous nonce map: {:?}", previous_nonce_per_address);
...

        {
            ...
            for block in txns.chunks(2500) {                
                ...
                let block_length = block_fixed.len();
                let block_id = aptos_crypto::HashValue::random();
                let output: StateComputeResult = executor.execute_block_benchmark_prototype(
                    (block_id, block_fixed), 
                    executor.committed_block_id(), 
                    &channels,
                    &mut previous_nonce_per_address
    ...
    // IMPORTANT (0xuki): transaction submission endpoint
    #[oai(
        path = "/transactions",
        method = "post",
...
// let res = ExecStatus::Success(
//     AptosTransactionOutput::new(
...
impl TransactionsApi {
    fn gen_move_handlers_and_spawn_worker(&self, worker_sender: WorkerSender) -> ManagerSender {
        let (tx_req, rx_req) = mpsc::channel::<ExecutionContext>();
        thread::Builder::new().name("move_execution_thread".to_string()).spawn (move || {
            println!("inside MOVE thread");
            loop {
                {
                    let received = rx_req.recv();
                    match received {
                        Ok(execution_context) => {
                            let ExecutionContext { base_view, executor, execution_state_cache, idx, txn_batch } = execution_context;
                            let mut execution_results = Vec::with_capacity(txn_batch.len());
                            
                            // println!("Received a txn_batch of size {} in aptos handler", txn_batch.len());
                            for txn in txn_batch {
                                let res: ExecStatus<AptosTransactionOutput, VMStatus> = 
                                    executor.execute_transaction(
                                        &LatestViewArc::<PreprocessedTransaction, CachedStateView>::new_btree_view(
                                            base_view.clone(), 
                                ...
                                match res {
                                    ExecStatus::Success(ref output) | ExecStatus::SkipRest(ref output) => {
                                        let write_set = output.get_writes().into_iter();
                                        {
                                            // Apply the writes.
                                            let mut executed_state_cache_writer = execution_state_cache.write().unwrap(); 
                                            for (ap, write_op) in write_set {
                                                (*executed_state_cache_writer).insert(ap, write_op);
                                            }
                                        }
                                        execution_results.push(ExecutionStatusMultiWorker {status: res, worker_type: WorkerType::Aptos});
                                    }
                                    _ => {
                                        println!("ERROR DURING EXECUTION");
                                        panic!("MOVE WORKER THREAD PANICKED");
                                    }
                                }
                            }
                            
                            std::mem::drop(executor);
                            std::mem::drop(base_view);

                            let reply_result = worker_sender.send(execution_results);
                            // match reply_result {
                            //     Ok(_) => println!("Sent back an executed native Aptos txn!"),
                            //     Err(e) => println!("Could not send reply in Move thread... Error: {e:?}")
                            // }
                        },
                        Err(e) => {
                            // println!("error: {}", e);
    ...
    fn gen_eth_handlers_and_spawn_worker(&self, worker_sender: WorkerSender) -> ManagerSender {
        let (tx_req, rx_req) = mpsc::channel::<ExecutionContext>();
        thread::Builder::new().name("eth_execution_thread".to_string()).spawn (move || {
            println!("inside ETH thread");
            loop {
                {
                    let received = rx_req.recv();
                    match received {
                        Ok(execution_context) => {
                            ...
                            // println!("Received a txn_batch of size {} in eth handler", txn_batch.len());
                            for txn in txn_batch {
                                let res: ExecStatus<AptosTransactionOutput, VMStatus> = 
                                    executor.execute_transaction(
                                        &LatestViewArc::<PreprocessedTransaction, CachedStateView>::new_btree_view(
                                ...
                                match res {
                                    ExecStatus::Success(ref output) | ExecStatus::SkipRest(ref output) => {
                                        ...
                                        execution_results.push(ExecutionStatusMultiWorker {status: res, worker_type: WorkerType::Ethereum});
                                    }
                                    _ => {
                                        println!("ERROR DURING EXECUTION in ETH thread");
                                        panic!("ETH WORKER THREAD PANICKED");
                                    }
                                }
                            }

                            // dbg!("Number of arc references left (called from ETH thread): {}", Arc::strong_count(&base_view_clone));
                            std::mem::drop(executor);
                            std::mem::drop(base_view);

                            let reply_result = worker_sender.send(execution_results);
                            // match reply_result {
                            //     Ok(_) => println!("Sent back an executed native Ethereum txn!"),
                            //     Err(e) => println!("Could not send reply in Eth thread... Error: {:?}", e)
                            // };
    ...
    fn gen_cross_handlers_and_spawn_worker(&self, worker_sender: WorkerSender) -> ManagerSender {
        let (tx_req, rx_req) = mpsc::channel::<ExecutionContext>();
        thread::Builder::new().name("cross_execution_thread".to_string()).spawn(move || {
            println!("inside CROSS thread");
            loop {            
                {
                    let received = rx_req.recv();
                    match received {
                        Ok(execution_context) => {
                            ...
                            // println!("Received a txn_batch of size {} in cross handler", txn_batch.len());
                            for txn in txn_batch {
                                let res: ExecStatus<AptosTransactionOutput, VMStatus> = 
                                    executor.execute_transaction(
                                        &LatestViewArc::<PreprocessedTransaction, CachedStateView>::new_btree_view(
                                ...
                                match res {
                                    ExecStatus::Success(ref output) | ExecStatus::SkipRest(ref output) => {
                                        ...
                                        execution_results.push(ExecutionStatusMultiWorker {status: res, worker_type: WorkerType::Cross});
                                    }
                                    _ => {
                                        println!("ERROR DURING EXECUTION in CROSS thread");
                                        panic!("CROSS WORKER THREAD PANICKED");
                                    }
                                }
                            }
                            ...
                            let reply_result = worker_sender.send(execution_results);
                            // match reply_result {
                            //     Ok(_) => println!("Sent back an executed cross txn!"),
                            //     Err(e) => println!("Could not send reply in Cross thread... Error: {:?}", e)
                            // };
    ...
    fn read_tx_with_tx_type_padding(
        &self, 
        bytes: &Vec<u8>, 
        bytes_read: &mut usize,
        previous_nonce_per_address: &mut HashMap<AccountAddress, u64>,
        no_setup_tx_ethereum: &mut bool,
        no_setup_tx_aptos: &mut bool,
    ) -> SignedTransaction {
        // size of (tx and its type) fits in 2 bytes 
        let first_byte = ((*bytes)[*bytes_read + 1] as u32) << 8;
        let tx_and_tx_type_length: u32 = (first_byte + bytes[*bytes_read] as u32) as u32;  
        *bytes_read += 2;
        
        let tx_type_length: u32 = 1;
        let tx_bytes: &[u8] = &(*bytes)[*bytes_read..*bytes_read + tx_and_tx_type_length as usize - tx_type_length as usize];
        *bytes_read += tx_and_tx_type_length as usize - tx_type_length as usize;

        let encoded_tx_type = (*bytes)[*bytes_read];
        *bytes_read += 1;

        // println!("Byte-Encoded tx type: {encoded_tx_type}");

        fn there_were_setup_aptos_txs(no_setup_tx_aptos: &mut bool) -> bool {
            !*no_setup_tx_aptos
        }

        fn there_were_setup_ethereum_txs(no_setup_tx_ethereum: &mut bool) -> bool {
            !*no_setup_tx_ethereum
        }
        

        let ledger_info = self.context.get_latest_ledger_info::<SubmitTransactionError>().unwrap();
        
        let transaction = match encoded_tx_type {
            1 | 3 => {
                let mut aptos_tx: SignedTransaction = self.get_signed_transaction(
                    &ledger_info,
                    SubmitTransactionPost::Bcs(Bcs(Vec::from(tx_bytes)))
                ).unwrap();

                match encoded_tx_type {
                    1 => aptos_tx.transaction_type = TxType::AptosTx,
                    3 => aptos_tx.transaction_type = TxType::CrossTxAptosOrigin, 
                    _ => panic!("Impossible...")
                }

                if aptos_tx.sequence_number() == 0 {
                    *no_setup_tx_aptos = true;
                } else if there_were_setup_aptos_txs(no_setup_tx_aptos) {
                    previous_nonce_per_address.entry(aptos_tx.sender()).or_insert(aptos_tx.sequence_number() - 1);
                }
                aptos_tx
            }
            5 | 7 => {
                let eth_tx: EthTransaction = rlp::decode(tx_bytes).map_err(Error::to_call_error).expect("Decoding of eth tx was not successful (native)");
                let mut signed_tx = SignedTransaction::try_from(eth_tx).expect("Conversion to eth tx was not successful (native)"); 
                
                match encoded_tx_type {
                    5 => signed_tx.transaction_type = TxType::EthereumTx,
                    7 => signed_tx.transaction_type = TxType::CrossTxEthereumOrigin, 
                    _ => panic!("Impossible...")
                }

                if signed_tx.sequence_number() == 0 {
                    *no_setup_tx_ethereum = true;
                } else if there_were_setup_ethereum_txs(no_setup_tx_ethereum) {
                    previous_nonce_per_address.entry(signed_tx.sender()).or_insert(signed_tx.sequence_number() - 1);
                }
                signed_tx 
            }
            _ => {
                panic!("Bad transaction type supplied... A bug in transaction generation or reading process!");
            }
        };            

        // println!("Transaction sender: {}", transaction.sender());

        transaction
    }

    // #[allow(dead_code)]
    fn read_tx_partially_ignoring_tx_type_padding(
        &self, 
        bytes: &Vec<u8>, 
        bytes_read: &mut usize,
    ) -> SignedTransaction {
    ...
    // #[allow(dead_code)]
    fn read_tx_without_tx_type_padding(&self, bytes: &Vec<u8>, bytes_read: &mut usize) -> SignedTransaction {
        let first_byte = ((*bytes)[*bytes_read + 1] as u32) << 8;
        let tx_length: u32 = (first_byte + bytes[*bytes_read] as u32) as u32;
        println!("Read tx with size {}", tx_length);
        *bytes_read += 2;
        let tx_bytes: &[u8] = &(*bytes)[*bytes_read..*bytes_read + tx_length as usize];
        let ledger_info = self.context.get_latest_ledger_info::<SubmitTransactionError>().unwrap();
        let aptos_tx: SignedTransaction = self.get_signed_transaction(
            &ledger_info,
            SubmitTransactionPost::Bcs(Bcs(Vec::from(tx_bytes)))
        ).unwrap();
        *bytes_read += tx_length as usize;
        aptos_tx
    }
    ...
    pub async fn simulate(
        ...
        let (_, output_ext, _) = AptosVM::simulate_signed_transaction(&txn, &move_resolver);
...
fn override_gas_parameters(
    ...
    SignedTransaction::new_with_authenticator(raw_txn, signed_txn.authenticator(), TxType::PlaceholderTypeTx)
}
