...
use cfx_primitives::{
    transaction::TxType, Eip155Transaction, SignedTransaction as EthSignedTransaction, Transaction as EthRawTransaction, TransactionWithSignature as EthTransaction
};
...
use aptos_crypto::secp256k1::{Secp256k1PublicKey, Secp256k1Signature};
#[cfg(any(test, feature = "fuzzing"))]
pub use change_set::NoOpChangeSetChecker;
pub use change_set::{ChangeSet, CheckChangeSet};
pub use ethereum::{EthAddress, EthTransactionPayload};
...
impl RawTransaction {
    ...
    /// Create a new `RawTransaction` that represents an Ethereum transaction.
    ///
    /// An Eth transaction will be executed with EVM instead of MoveVM.
    pub fn new_eth(
        sender: AccountAddress,
        sequence_number: u64,
        eth_payload: EthTransactionPayload,
        max_gas_amount: u64,
        gas_unit_price: u64,
        expiration_timestamp_secs: u64,
        chain_id: ChainId,
        ...
        RawTransaction {
            sender,
            sequence_number,
            payload: TransactionPayload::EthTransactionPayload(eth_payload),
            max_gas_amount,
            gas_unit_price,
            expiration_timestamp_secs,
            chain_id,
    ...
    pub fn format_for_client(&self, get_transaction_name: impl Fn(&[u8]) -> String) -> String {
        let (code, args) = match &self.payload {
            ...
            TransactionPayload::EthTransactionPayload(_) => ("eth tx".to_string(), vec![]),
        };
    ...
    /// IMPORTANT(0xuk): get sender of the transaction
    ...
    pub fn eth_transaction(&self) -> Option<EthRawTransaction> {
        let TransactionPayload::EthTransactionPayload(EthTransactionPayload{ value, action, data }) = &self.payload else { return None };
        Some(
            Eip155Transaction {
                nonce: self.sequence_number.into(),
                gas_price: self.gas_unit_price.into(),
                gas: self.max_gas_amount.into(),
                action: action.into(),
                value: *value,
                chain_id: Some(self.chain_id.id() as u32),
                data: data.clone(),
            }
            .into(),
        )
...
pub enum TransactionPayload {
    ...
    /// A transaction that represents an Ethereum transaction.
    EthTransactionPayload(EthTransactionPayload),
}
...
impl SignedTransaction {
    pub fn new(
        ...
        SignedTransaction {
            ...
            authenticator,
            size: OnceCell::new(),
            transaction_type: TxType::PlaceholderTypeTx
        }
    }
    ...
    pub fn committed_hash_ref(&self) -> HashValue {
        let self_clone = self.clone();
        Transaction::UserTransaction(self_clone).hash()
    }

    pub fn eth_transaction(&self) -> Option<EthTransaction> {
        let raw_eth_tx = self.raw_txn.eth_transaction()?;
        let eth_signature = match &self.authenticator {
            TransactionAuthenticator::Secp256k1 { signature, .. } => signature.signature().clone(),
            _ => return None,
        };
        Some(raw_eth_tx.with_signature(eth_signature))
    }
}
...
impl TryFrom<EthTransaction> for SignedTransaction {
    type Error = Error;

    // Important (0xuk): API ETH transaction to System Digestible Tx
    fn try_from(eth_tx: EthTransaction) -> Result<Self> {
        let public = eth_tx
            .recover_public()
            .map_err(|e| anyhow!("recover_public error: e={:?}", e))?;
        let eth_sender = eth_public_to_address(&public);
        let aptos_sender = eth_address_to_aptos_address(&eth_sender);
        let payload = EthTransactionPayload::new(
            *eth_tx.value(),
            eth_tx.action().clone().into(),
            eth_tx.data().clone(),
        );
        // FIXME(lpl): Check if conversion overflow (u256 to u64 and u32 to u8) is an issue here.
        let raw_tx = RawTransaction::new_eth(
            aptos_sender,
            eth_tx.nonce().as_u64(),
            payload,
            eth_tx.gas().as_u64(),
            eth_tx.gas_price().as_u64(),
            u64::MAX,
            // FIXME(lpl): Check chain_id somewhere because it panics with 0.
            ChainId::new(eth_tx.chain_id().unwrap_or(ChainId::test().id() as u32) as u8),
        );
        let authenticator = TransactionAuthenticator::secp256k1(
            Secp256k1PublicKey::from_public(public),
            Secp256k1Signature::from_signature(eth_tx.signature()),
        );
...
/// The difference from `TryFrom<EthTransaction>` is that `sender` is not recovered from
/// `public`, so a fake-signed Eth transaction can be converted correctly.
impl TryFrom<EthSignedTransaction> for SignedTransaction {
    type Error = Error;

    fn try_from(value: EthSignedTransaction) -> Result<Self> {
        let sender = eth_address_to_aptos_address(&value.sender);
        let mut tx: SignedTransaction = value.transaction.try_into()?;
        tx.raw_txn.sender = sender;
        Ok(tx)
    }
}

const_assert!(AccountAddress::LENGTH >= EthAddress::len_bytes());

pub fn eth_address_to_aptos_address(eth_address: &EthAddress) -> AccountAddress {
    let mut bytes = [0u8; AccountAddress::LENGTH];
    bytes[0..EthAddress::len_bytes()].copy_from_slice(eth_address.as_bytes());
    AccountAddress::new(bytes)
}

pub fn aptos_address_to_eth_address(aptos_address: &AccountAddress) -> EthAddress {
    let mut bytes = [0u8; EthAddress::len_bytes()];
    bytes.copy_from_slice(&aptos_address.as_ref()[0..EthAddress::len_bytes()]);
    EthAddress::from(bytes)
}

pub fn eth_public_to_address(public: &Public) -> EthAddress {
    let mut keccak = Keccak::v256();
    let mut hash = [0u8; 32];
    keccak.update(public.as_ref());
    keccak.finalize(&mut hash);
    let mut result = EthAddress::zero();
