...
use aptos_evm::{EvmState, ViewWrapper, U256};
...
use aptos_types::{
    ...
    transaction::{
        authenticator::AuthenticationKey, ChangeSet, EthAddress, Transaction, WriteSetPayload,
    },
...
pub fn encode_genesis_transaction(
    ...
    Transaction::GenesisTransaction(WriteSetPayload::Direct(encode_genesis_change_set(
        ...
        consensus_config,
        gas_schedule,
        maybe_evm_genesis_account,
        maybe_move_genesis_account
    )))
}
...
pub fn encode_genesis_change_set(
    ...
    gas_schedule: &GasScheduleV2,
    maybe_evm_genesis_account: Option<EthAddress>,
    _maybe_move_genesis_account: Option<AccountAddress>,
) -> ChangeSet {
    validate_genesis_config(genesis_config);
    ...
    // IMPORTANT (0xuki): ADD TOKENS TO EVM ACCOUNT HERE!
    if let Some(evm_genesis_account) = maybe_evm_genesis_account {
        // Generate EVM genesis state.
        let state_view = GenesisStateView::new();
        let data_cache = state_view.as_move_resolver();
        ...
        state
            .state
            .add_balance(
                &evm_genesis_account.with_evm_space(),
        ...
        state.state.commit(Default::default(), None).unwrap();
        // Question: why drop the state
        drop(state);
        let evm_write_set = WriteSetMut::new(view_wrapper.drain());

        // println!(
        //     "initialize evm account: address={:?} write_set={:?}",
        //     evm_genesis_account, evm_write_set
        // );

        let (write_set, events) = change_set.into_inner();
        let merged_write_set = write_set
            .into_mut()
            .squash(evm_write_set)
            .unwrap()
            .freeze()
            .unwrap();
