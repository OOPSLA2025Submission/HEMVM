...
use aptos_block_executor::{aptos_vm::AptosVM, cached_state_view::{CachedStateView, StateCache}, executed_trees::ExecutedTrees, executor::{TwoWayChannels}, VMExecutor};
use std::sync::Arc;
...
use aptos_types::{
    ...
    transaction::{
        ExecutionStatus, Transaction, TransactionOutput, TransactionPayload, TransactionStatus,
    },
...
impl ChunkOutput {
    ...
    pub fn by_transaction_execution_prototype<V: VMExecutor>(
        transactions: Vec<Transaction>,
        state_view: CachedStateView,
    ...
    where
    {
        ...
        // println!("Number of arc references left: {}", Arc::strong_count(&state_view_arc));
        let transaction_outputs: Vec<TransactionOutput> = Self::execute_block_prototype::<V>(
            transactions.clone(), 
        ...
        Ok(Self {
            transactions,
            transaction_outputs,
            state_cache: Arc::try_unwrap(state_view_arc).expect("couldn't unwrap state view arc").into_state_cache(),
        })
    }
    ...
    // Important (0xuki): block execution!
    /// Executes the block of [Transaction]s using the [VMExecutor] and returns
    /// a vector of [TransactionOutput]s.
    #[cfg(not(feature = "consensus-only-perf-test"))]
    fn execute_block_prototype<V: VMExecutor>(
        transactions: Vec<Transaction>,
        ...
        if let Transaction::UserTransaction(user_txn) = txn {
            match user_txn.payload() {
                ...
                TransactionPayload::EthTransactionPayload(_) => {
                    metrics::APTOS_PROCESSED_USER_TRANSACTIONS_PAYLOAD_TYPE
                        .with_label_values(&[process_type, "eth", state])
                        .inc();
                },
            }
        }
