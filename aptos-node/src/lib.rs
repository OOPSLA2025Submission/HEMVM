...
use aptos_types::{chain_id::ChainId, transaction::EthAddress};
use clap::Parser;
use ethrpc::bootstrap as bootstrap_eth_api;
...
pub fn setup_test_environment_and_start_node<R>(
    ...
    rng: R,
    maybe_evm_genesis_account: Option<EthAddress>,
    maybe_move_genesis_account: Option<AccountAddress>
) -> anyhow::Result<()>
where
    ...
    let config = if validator_config_path.exists() {
        ...
        let builder = aptos_genesis::builder::Builder::new(&test_dir, framework.clone())?
            ...
            .with_init_genesis_config(Some(Arc::new(move |genesis_config| {
                ...
                genesis_config.maybe_evm_genesis_account = maybe_evm_genesis_account;
                genesis_config.maybe_move_genesis_account = maybe_move_genesis_account;
            })))
...
pub fn setup_environment_and_start_node(
    ...
    let (_aptos_db, db_rw, backup_service, genesis_waypoint) =
    ...
    // IMPORTANT(0xuki): setting up benchmark mutex so that native execution blocks on experiment run
    let benchmark_mutex = Arc::new(tokio::sync::Mutex::new(()));
    // Bootstrap the API and indexer
    let (mempool_client_receiver, api_runtime, index_runtime) =
        services::bootstrap_api_and_indexer(&node_config, Arc::new(db_rw.clone()), chain_id, benchmark_mutex.clone())?;

    if let Some((runtime, context)) = &api_runtime {
        // TODO(lpl): Use a different runtime/context or not?
        bootstrap_eth_api(context.clone(), runtime.handle())?;
        Some(runtime)
    } else {
        None
    };
    ...
    Ok(AptosHandle {
        _api_runtime: api_runtime.map(|x| x.0),
