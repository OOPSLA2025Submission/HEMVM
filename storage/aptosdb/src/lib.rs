...
use aptos_block_executor::{
    executed_trees::ExecutedTrees, state_delta::StateDelta, state_view::{DbReader, DbStateView, DbWriter, Order, StateSnapshotReceiver}
};
...
use aptos_block_executor::data_cache::AsMoveResolver;
            ...
            let t1 = s.spawn(|| {
                ...
                zip_eq(first_version..=last_version, txns_to_commit)
                    .map(|(ver, txn_to_commit)| {
                        // Index the block metadata event by the block hash.
                        if let Transaction::BlockMetadata(block_metadata) =
                            txn_to_commit.transaction()
                        {
                            self.event_store
                                .put_block_version_by_hash(block_metadata.id(), ver)?;
                        }
...
impl DbReader for AptosDB {
    ...
    // IMPORTANT (0xuki): get latest ledger info call
    ...
    fn get_transaction_by_eth_hash(
        &self,
        hash: HashValue,
        ledger_version: Version,
        fetch_events: bool,
    ) -> Result<Option<TransactionWithProof>> {
        gauged_api("get_transaction_by_eth_hash", || {
            self.transaction_store
                .get_transaction_version_by_eth_hash(&hash, ledger_version)?
                .map(|v| self.get_transaction_with_proof(v, ledger_version, fetch_events))
                .transpose()
        })
    }
    ...
    fn get_block_version_by_hash(&self, block_hash: HashValue) -> Result<Option<Version>> {
        gauged_api("get_block_version_by_hash", || {
            self.event_store.get_block_version_by_hash(block_hash)
        })
    }
