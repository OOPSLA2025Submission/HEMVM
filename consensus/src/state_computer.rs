...
pub struct ExecutionProxy {
    ...
    benchmark_mutex: Arc<AsyncMutex<()>>
}
...
impl StateComputer for ExecutionProxy {
    async fn compute(
        ...
        let _result = self.benchmark_mutex.lock().await;
        ...
        // println!("transactions_to_execute size {}", transactions_to_execute.len());

        // Important (0xuki): execution of the block starts here (task spawning -> get inside async)
        let compute_result = monitor!(
            "execute_block",
            tokio::task::spawn_blocking(move || {
                executor.execute_block((block_id, transactions_to_execute), parent_block_id) // executor type = BlockExecutor
            })
            .await
        )
